...
function App() {
  ...
  const handleClick = (e) => {
    ...
    if (buttonName === "1/x") {
      try {
        ...
        const reciprocal = 1 / parsedResult;
        setResult(reciprocal.toString());
      } catch {
        setResult("Error");
      }
    }
      ...
      try {
        const parsedResult = parseFloat(result);
        const square = Math.pow(parsedResult, 2);
  ...
  return (
    <div className="App">
      <h1>Calculator by Vishwas Premchandani</h1>
      ...
      <div className='container'>
        ...
        <div className='keypad'>
          ...
          <button name="1/x" onClick={handleClick} className='highlight'>1/x</button>
          <button name="x^2" onClick={handleClick} className='highlight'>x<sup>2</sup></button>
        </div>
